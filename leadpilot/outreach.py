...
class OutreachGenerator:
    ...
    def generate(self, lead: Lead, sender_niche: str = "") -> str:
        ...
        prompt = (
            ...
            "Каждый абзац должен состоять из одного-двух коротких предложений. "
            "Сохраняй все пять смысловых частей, но не повторяй одну мысль разными "
            "словами. Начни с приветствия и закончи мягким вопросом или предложением "
            "показать идею подробнее. Общая длина — примерно 600–800 знаков.\n\n"
    ...
    def fallback(cls, lead: Lead, sender_niche: str = "") -> str:
        ...
        return (
            ...
            "видно, что вы уже выстраиваете удобное взаимодействие с клиентами.\n\n"
            "Могу ошибаться, но у компаний вашего направления часть обращений иногда "
            "теряется или получает ответ не сразу, особенно когда запросов становится больше.\n\n"
            "В таком случае потенциальный клиент может не дождаться ответа, не понять "
            "следующий шаг и выбрать другую компанию.\n\n"
            "Обычно это можно закрыть быстрым первым ответом и простой системой, которая "
            "помогает контролировать заявки и не терять диалоги.\n\n"
            f"{niche} Поэтому решил написать именно вам: здесь мой опыт может быть "
            "полезен. Будет уместно, если я коротко покажу идею на вашем примере?"
        )
